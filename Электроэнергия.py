...
###############################################################################
FINREZ =FINREZ.copy()
FINREZ["значение_фрс"] = FINREZ["значение_фрс"].replace(",", ".", regex=True)
FINREZ["значение_фрс"] = FINREZ["значение_фрс"].fillna(0)
FINREZ["значение_фрс"] = FINREZ["значение_фрс"].astype(float)

FINREZ["дата"] = FINREZ["дата"].dt.date
FINREZ = FINREZ.pivot(index=['магазин'], columns="дата", values='значение_фрс')
FINREZ = FINREZ.reset_index()
to_exel(x=FINREZ, name="Электроэнергия_общий")
